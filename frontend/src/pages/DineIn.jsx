  ...
  return (
    <div style={{
      ...
      {isOnline
        ...
        : '📴 You are offline — Menu loaded from cache'}
    </div>
  );
}
...
export default function DineIn() {
  ...
  const loadTables = useCallback(async () => {
    ...
    try {
      ...
      const freeTables = t.filter(t => t.table_status === 'free' && t.is_active);
      setTables(freeTables);
      cacheTables(t); // cache all tables
    } catch {
      // load from cache if offline
      const cached = getCachedTables();
      if (cached.length > 0) {
        setTables(cached.filter(t => t.table_status === 'free' && t.is_active));
        showToast('📴 Showing cached tables (offline)', 'info');
      }
    }
  }, [cid]);
  ...
  const selectTable = async (table) => {
    if (!isOnline) {
      showToast('📴 Cannot create order while offline', 'error');
      return;
    }
    setLoading(true);
    try {
  ...
  const searchCustomer = async () => {
    if (!phone) return;
    if (!isOnline) {
      showToast('📴 Customer search not available offline', 'error');
      return;
    }
    try {
  ...
  const saveNewCustomer = async () => {
    if (!isOnline) {
      showToast('📴 Cannot save customer while offline', 'error');
      return;
    }
    try {
  ...
  const addItem = async (menu) => {
    ...
    if (!isOnline) {
      ...
      showToast('📴 Item added (offline — will sync later)', 'info');
      return;
    }
    try {
  ...
  if (step === 'tables') return (
    <div style={S.page}>
      <OfflineBanner />
      {!isOnline && (
        <div style={{ background: '#fff3cd', border: '1px solid #ffc107', borderRadius: 8, padding: '10px 16px', margin: '16px 16px 0', fontSize: 13, color: '#856404' }}>
          📴 <strong>Offline Mode</strong> — Showing cached tables. Order creation requires internet.
        </div>
      )}
      <div style={S.header}>
      ...
      <div style={S.tableGrid}>
        ...
        {tables.map(t => (
          <div key={t.table_id} style={{ ...S.tableCard, opacity: !isOnline ? 0.6 : 1 }} onClick={() => !loading && selectTable(t)}>
            ...
            <div style={S.freeTag}>{isOnline ? 'Free' : '📴 Offline'}</div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={S.left}>
        <div style={S.orderHeader}>
          <div>
            ...
            {!isOnline && <div style={{ fontSize: 11, color: '#dc2626', marginTop: 2 }}>📴 Offline</div>}
          </div>
        ...
        <div style={S.custBox}>
          ...
          {customer && (
            <div style={S.custFound}>✅ {customer.name} — {customer.phone}</div>
          )}
          {showCustForm && (
            <div style={S.custForm}>
              ...
              <button style={{ ...S.saveBtn, opacity: !isOnline ? 0.5 : 1 }} onClick={saveNewCustomer} disabled={!isOnline}>
                {isOnline ? 'Save Customer' : '📴 Offline'}
              </button>
            </div>
          )}
        </div>
            ...
            <div key={m.food_menu_id || m.foodmenuid} style={S.menuCard}>
              <div style={{ position: 'relative', width: '100%', marginBottom: '6px' }}>
                ...
                <div style={{
                  ...
                  background: m.is_veg !== false ? '#16a34a' : '#dc2626',
