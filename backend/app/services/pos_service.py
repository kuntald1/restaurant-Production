"""
app/services/pos_service.py  — COMPLETE FINAL VERSION WITH ALL ROUNDING FIXES
"""
...
def _assert_items_editable(order: Order):
    ...
    if locked:
        ...
        raise HTTPException(400, f"Cannot modify: kitchen has started preparing — {names}.")
...
def create_order(db: Session, data: OrderCreate):
    ...
    if data.order_type.value == "dine_in" and data.table_id:
        ...
        if table.table_status == TableStatusEnum.reserved:
            raise HTTPException(400, f"Table {table.table_name} is reserved.")
        ...
        if remaining_seats <= 0:
            raise HTTPException(400, f"Table {table.table_name} is full ({table.seats}/{table.seats} seats occupied).")
        if covers_requested > remaining_seats:
            raise HTTPException(400, f"Table {table.table_name} only has {remaining_seats} seat(s) remaining, but you requested {covers_requested}.")
        ...
        if s_amount and float(s_amount) > 0:
            surcharge_amount = float(s_amount) * covers_requested if s_type == "per_cover" else float(s_amount)
    ...
    for item_data in (data.items or []):
        ...
        item = OrderItem(
            ...
            total_price       = rounded_price * item_data.quantity,
...
def add_item_to_order(db: Session, order_id: int, item_data: OrderItemCreate, company_id: int):
    ...
    # Create new row (different item or already in KOT)
    item = OrderItem(
        order_id          = order_id,
...
def update_item_quantity(db: Session, order_id: int, order_item_id: int, quantity: int):
    ...
    if item.kot_item_status == KOTItemStatusEnum.kot_inprocess:
        raise HTTPException(400, f"'{item.item_name}' is being prepared. Cannot change quantity.")

    if quantity <= 0:
        item.is_cancelled     = True
        item.cancelled_reason = "Removed by staff"
    else:
        rounded_price    = round(float(item.unit_price))
        item.quantity    = quantity
        item.unit_price  = rounded_price
        item.total_price = rounded_price * quantity
...
def create_kot(db: Session, data: KOTCreate):
    ...
    table_name = None
    if order.table_id:
        tbl = db.query(RestaurantTable).filter(RestaurantTable.table_id == order.table_id).first()
...
def generate_bill(db: Session, data: BillCreate):
    ...
    bill_kwargs = dict(
        ...
        total_payable     = total_rounded,
        amount_paid       = amount_paid_r,
        payment_method    = data.payment_method,
        payment_reference = data.payment_reference,
        is_paid           = amount_paid_r >= total_rounded,
        paid_at           = datetime.utcnow() if amount_paid_r >= total_rounded else None,
    ...
    if customer_id:
        try:
            ...
            if crm_cust:
                ...
                crm_cust.total_spend  = float(crm_cust.total_spend or 0) + float(total_rounded)
